                        ...
                        EDGEBOUNDS* edg)
{
   ...
   for (k = 0; k < N; ++k)
   {
      ...
      /* IF NEXT EDGEBOUND IS NOT ON THE SAME ROW (ROW COMPLETED), UPDATE SPECIAL STATES */
      if (edg->bounds[N+1].diag != x)
      {
         /* SPECIAL STATES */
         /* J state */
         sc_1 = XMX(SP_J,i-1) + XSC(SP_J,SP_LOOP);       /* J->J */
                        ...
                        EDGEBOUNDS* edg)
{
   ...
   for (k = N-1; k >= 0; --k)
   {
      ...
      for (j = y2-1; j >= y1; --j)
      {
         ...
         // /* NOTE: SAVE FOR DEBUGGING (transition states slightly off?) */
         // printf("(%d, %d): ", i, j);
