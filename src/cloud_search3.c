                           ...
                           EDGEBOUNDS* edg)
{
   ...
   int    x, y1, y2;
   int    row_cur, row_prev;
   int    r_0b, r_0e, r_1b, r_1e;
   int    r_0, r_1;
   ...
   for (k = 0; k < N;)
   {
      /* get start of current row */
      row_cur = edg->bounds[k].diag;
      r_0 = row_cur % 2;
      r_0b = k;
      ...
      for (i = r_0b; i < r_0e; i++)
      {
         ...
         MMX3(i, 0) = IMX3(i, 0) = DMX3(i, 0) = -INF;
         // XMX(SP_E,i) = -INF;
                            ...
                            EDGEBOUNDS* edg)
{
   ...
   for (k = N - 1; k >= 0; --k)
   {
      ...
      if (N > 0 && edg->bounds[N - 1].diag != x)
      {
         /* SPECIAL STATES */
         XMX(SP_B, i) = MMX3(i + 1, 1) + TSC(0, B2M) + MSC(1, A);
